...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ic_doge_canister::memory::set_memory(ic_stable_structures::FileMemory::new(File::open(
        &args.input,
    )?));
    ...
    if process_utxos {
        ...
        log!(
            "Extracted {} large UTXOs from canister state",
            large_utxo_count
        );
    }
    ...
    if process_balances {
        ...
        canister_data
            .balances
            .retain(|(_address, balance)| *balance != 0);
        log!(
            "Filtered out {} addresses with 0 balance (kept {})",
    ...
    log!(
        "  Sorting {} block headers...",
        canister_data.block_headers.len()
    );
    canister_data.block_headers.sort_by(|a, b| a.0.cmp(&b.0));

    log!(
        "  Sorting {} block heights...",
        canister_data.block_heights.len()
    );
    canister_data.block_heights.sort_by(|a, b| a.0.cmp(&b.0));
    ...
    let (address_utxos_hash, address_balance_hash) = if process_balances {
        ...
        log!(
            "  Computing address balances hash ({} entries)...",
            canister_data.balances.len()
        );
    ...
    let (block_headers_hash, block_heights_hash) = if process_headers {
        ...
        log!(
            "  Computing block heights hash ({} entries)...",
            canister_data.block_heights.len()
        );
...
fn check_invariants(data: &CanisterData, utxos: &[Utxo]) -> Result<(), String> {
    ...
    let utxo_height_zero_count = utxos
        .iter()
        .filter(|addr_utxo| addr_utxo.height == 0)
        .count();
    if utxo_height_zero_count > 0 {
        return Err(format!(
            "Found {} UTXOs at height 0, expected none",
            utxo_height_zero_count
        ));
    }
    log!("  ✓ No UTXOs at height 0");

    // Check for undersized block headers
    let header_sizes: Vec<usize> = data
        .block_headers
        .iter()
    ...
    if undersized_count > 0 {
        return Err(format!(
            "Found {} block headers smaller than 80 bytes, expected none",
            undersized_count
        ));
    }
    ...
    if zero_header_count > 0 {
        return Err(format!(
            "Found {} zero block headers, expected none",
            zero_header_count
        ));
    }
    ...
    log!(
        "  ✓ Header and height counts match ({} entries)",
        headers_count
    );
    ...
    let unique_hashes: HashSet<&BlockHash> = data
        .block_headers
        .iter()
    ...
    if height_span as usize != heights.len() {
        return Err(format!(
            "Missing blocks: expected {} blocks from {} to {}, found {}",
            height_span,
            min_height,
            max_height,
            heights.len()
    ...
    log!(
        "  ✓ Block height continuity verified ({} to {} = {} blocks)",
        min_height,
        max_height,
        height_span
    );
...
fn print_statistics(data: &CanisterData, utxos: &[Utxo]) {
    ...
    if !utxos.is_empty() {
        ...
        println!(
            "  Total Value: {} DOGE",
            total_value_doge.separated_string()
        );
        ...
        println!(
            "  UTXO Height Range: {} - {}",
            min_height.separated_string(),
            max_height.separated_string()
        );
        ...
        let medium_count = script_sizes
            .iter()
            .filter(|&&size| size > 25 && size <= 201)
            .count();
        ...
        let mut values_doge: Vec<f64> = utxos
            .iter()
    ...
    println!(
        "\n  Total Address UTXOs entries: {}",
        data.address_utxos.len().separated_string()
    );

    if !data.address_utxos.is_empty() {
        println!(
            "\nFirst {} Address UTXO Details:",
        ...
        for addr_utxo in &data.address_utxos {
            *address_counts
                .entry(addr_utxo.address.to_string())
                .or_insert(0) += 1;
        ...
        let median_utxos_per_addr = if counts.is_empty() {
            0
        } else {
            counts[counts.len() / 2]
        };
        ...
        println!(
            "\n  Unique addresses: {}",
            unique_addresses.separated_string()
        );
        println!(
            "  UTXOs per address - Min: {}, Max: {}, Avg: {:.1}, Median: {}",
    ...
    println!(
        "\n  Total Address Balances entries: {}",
        balance_count.separated_string()
    );

    if !data.balances.is_empty() {
        let mut balances_satoshis: Vec<u128> =
            data.balances.iter().map(|(_, balance)| *balance).collect();
        let mut balances_doge: Vec<f64> = balances_satoshis
            .iter()
            .map(|&b| b as f64 / 100_000_000.0)
            .collect();
        ...
        println!(
            "\n  Total Supply: {} DOGE",
            total_supply_doge.separated_string()
        );
        ...
        println!(
            "    Median:  {:.8} DOGE",
            balances_doge[balances_doge.len() / 2]
        );
        ...
        let dust_count = balances_satoshis
            .iter()
            .filter(|&&b| b > 0 && b < dust_threshold)
            .count();
        let small_count = balances_satoshis
            .iter()
            .filter(|&&b| b >= dust_threshold && b < small_threshold)
            .count();
        let medium_count = balances_satoshis
            .iter()
            .filter(|&&b| b >= small_threshold && b < medium_threshold)
            .count();
        let large_count = balances_satoshis
            .iter()
            .filter(|&&b| b >= medium_threshold && b < large_threshold)
            .count();
        let whale_count = balances_satoshis
            .iter()
            .filter(|&&b| b >= large_threshold)
            .count();
        ...
        let zero_balance_count = balances_satoshis
            .iter()
            .filter(|&&balance| balance == 0)
            .count();
        println!(
            "\n  Number of addresses with zero balance: {}",
            zero_balance_count
        );
        ...
        println!(
            "    Top 1% of addresses hold: {:.2}% of total supply",
            (top_1_wealth as f64 / total_supply as f64) * 100.0
        );
        println!(
            "    Top 5% of addresses hold: {:.2}% of total supply",
            (top_5_wealth as f64 / total_supply as f64) * 100.0
        );
        println!(
            "    Top 10% of addresses hold: {:.2}% of total supply",
    ...
    println!(
        "\n  Total block headers entries: {}",
        headers_count.separated_string()
    );
    println!(
        "  Total block heights entries: {}",
        heights_count.separated_string()
    );
    ...
    if !data.block_headers.is_empty() && !data.block_heights.is_empty() {
        ...
        println!(
            ...
            min_height.separated_string(),
            max_height.separated_string(),
            height_span.separated_string()
        );

        let mut header_sizes: Vec<usize> = data
            .block_headers
            .iter()
            .map(|(_, blob)| blob.as_slice().len())
            .collect();
        ...
        if auxpow_count > 0 {
            ...
            println!("\n  AuxPow Size Distribution Analysis:");
            println!(
                "    AuxPow data size range: {} - {} bytes",
                min_auxpow.separated_string(),
                max_auxpow.separated_string()
            );
            println!("    Mean AuxPow size: {:.1} bytes", mean_auxpow);
            println!(
                "    Median AuxPow size: {} bytes",
                median_auxpow.separated_string()
            );
            ...
            println!(
                "      25th percentile: {} bytes",
                p25_auxpow.separated_string()
            );
            println!(
                "      75th percentile: {} bytes",
                p75_auxpow.separated_string()
            );
            println!(
                "      90th percentile: {} bytes",
                p90_auxpow.separated_string()
            );
            println!(
                "      95th percentile: {} bytes",
                p95_auxpow.separated_string()
            );
            println!(
                "      99th percentile: {} bytes",
                p99_auxpow.separated_string()
            );
        }
        ...
        for h in heights.iter().rev().take(5) {
            ...
            println!("{:<64} {}", hash_hex, h.separated_string(),);
        }
    }
