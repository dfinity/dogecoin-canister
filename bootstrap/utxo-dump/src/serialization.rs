...
pub(crate) fn decompress_amount(compressed: u64) -> Result<u64, Error> {
    ...
    let result = n.checked_mul(10u64.pow(e)).ok_or_else(|| {
        Error::new(
            ErrorKind::InvalidData,
            "Amount overflow during decompression",
        )
    })?;
...
mod tests {
    ...
    fn test_read_varint_bitcoin_examples() {
        ...
        assert_eq!(
            test_read_varint(&[0x8A, 0x95, 0xC0, 0xBB, 0x00]).unwrap(),
            3000000000
        )
    }
