...
// Expected hash of utxo-dump output for Bitcoin mainnet 250k blocks chainstate data
// (present as `test-data/chainstate_btc_mainnet_250k.tar.gz`).
//
// The chainstate tar.gz file has been generated by syncing the Bitcoin blockchain up to the
// first 250k blocks:
// ```bash
...
// TODO
const SHA256_OUTPUT_DOGE_MAINNET_350_135: &str = "0123456789abcdef";
...
fn test_utxo_dump_dogecoin_mainnet_regression() {
    ...
    let test_chainstate_path = extract_chainstate_data(&temp_dir, "chainstate_doge_mainnet_350135.tar.gz")
        .expect("Failed to extract Dogecoin mainnet chainstate data. Make sure tests/test-data/chainstate_doge_mainnet_350135.tar.gz exists.");

    println!("Testing with Dogecoin mainnet chainstate (350,135 blocks)...");
