...
fn verify_with_excessive_target<T: HeaderValidator>(validator_mainnet: &T, header: &Header) {
...
fn verify_timestamp_rules<T: HeaderValidator + HeaderValidatorExt>(
    ...
    let mut header = Header {
        ...
        .unwrap(),
        time: store.get_with_height(height_start_header + 1).unwrap().time + 1, // Larger than median time past
